...
class APAGARVTController extends Controller
{
    ...
    public function __construct(APAGARVT $visita_tecnica)
    {
    ...
    public function show($id)
    {
        try {
            ...
            if (!$registro) {
                ...
                $registro['cliente_seguranca_medidas'] = APAGARVTControllerSegurancaMedida::where('visita_tecnica_id', '=', $id)->get();
                ...
                for($i=1; $i<=$numero_pavimentos; $i++) {
                    foreach ($ids_seguranca_medidas as $seguranca_medida_id) {
                        if (isset($request['seguranca_medida_id_' . $i . '_' . $seguranca_medida_id])) {
                            ...
                            $visita_tecnica_seguranca_medida = APAGARVTControllerSegurancaMedida::where('visita_tecnica_id', $id)->where('pavimento', $i)->where('seguranca_medida_id', $seguranca_medida_id)->get();

                            if ($visita_tecnica_seguranca_medida->count() == 1) {
                                APAGARVTControllerSegurancaMedida::where('visita_tecnica_id', $id)->where('pavimento', $i)->where('seguranca_medida_id', $seguranca_medida_id)->update($dadosAtual);

                                //gravar transacao
                                Transacoes::transacaoRecord(2, 2, 'visitas_tecnicas', $visita_tecnica_seguranca_medida[0], $dadosAtual);
                            } else {
                                APAGARVTControllerSegurancaMedida::create($dadosAtual);
