...
class SistemaPreventivoUpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'name' => [
                'required',
                Rule::unique('sistemas_preventivos')
                    ->ignore($this->route('id')) // ou $this->id dependendo da sua rota
                    ->where(fn ($query) => $query->where('medida_seguranca_id', $this->medida_seguranca_id)),
            ],
        ];
    }
